...
router.get('/',async (ctx)=>{
    ...
    var count= await  DB.count('product',{});
    var result=await DB.find('product',{},{},{
        page:page,
        pageSize:pageSize,
        sortJson:{
            'add_time':-1
    ...
    await  ctx.render('admin/product/index',{
        list: result,
        page:page,
...
//
//    await  ctx.render('admin/product/ueditor');
//
//})
//post接收数据
router.post('/doAdd', upload.single('img_url'),async (ctx)=>{
    ...
    let price=ctx.req.body.price.trim();
    let pic=ctx.req.body.price;
    ...
    //属性的简写
    let json={
        pid,catename,title,price,status,is_best,is_hot,is_new,keywords,description,content,img_url,add_time
    }

    var result=DB.insert('product',json);
...
router.get('/edit',async (ctx)=>{
    ...
    var catelist=await DB.find('productcate',{});
    //当前要编辑的数据
    var productlist=await DB.find('product',{"_id":DB.getObjectId(id)});
...
router.post('/doEdit', upload.single('img_url'),async (ctx)=>{
    ...
    if(img_url){
        var json={
            pid,catename,title,apriceuthor,status,is_best,is_hot,is_new,keywords,description,content,img_url
        }
    }else{
        var json={
            pid,catename,title,price,status,is_best,is_hot,is_new,keywords,description,content
        }
    }

    DB.update('product',{"_id":DB.getObjectId(id)},json);
